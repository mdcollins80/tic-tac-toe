...
var store = __webpack_require__(1);
var boardReset = __webpack_require__(6);
...
var onGetFinishedGamesSuccess = function onGetFinishedGamesSuccess(data) {
  $('#stats-over').text(data.games.length);
};

var onGetFinishedGamesFailure = function onGetFinishedGamesFailure() {
...
var onNewGameSuccess = function onNewGameSuccess(data) {
  ...
  gameApi.getGames().then(onGetGamesSuccess).catch(onGetGamesFailure);
  gameApi.getFinishedGames().then(onGetFinishedGamesSuccess).catch(onGetFinishedGamesFailure);
};
...
var onGetGameSuccess = function onGetGameSuccess(data) {
  ...
  var clicks = store.game.cells.filter(function (item) {
    return item !== '';
  });
...
var boardReset = function boardReset() {
  for (var i = 0; i < store.gameArray.length; i++) {
  ...
  if (store.board === 'on') {
    ...
    if (store.game.cells[move] !== '') {
      ...
      var turn = function turn() {
        return store.turn() === 1 ? 'X' : 'O';
      };
      $(this).text(turn);
      store.gameArray[move] = turn();
      // check for winner
      var over = gamelogic.checkWinner(store.gameArray);
      ...
      gameApi.updateGame(JSON.stringify(data)).then(gameUi.onUpdateGameSuccess).catch(gameUi.onUpdateGameFailure);
    }
  }
};
...
var onGetGame = function onGetGame(event) {
  ...
  gameApi.getGame(data).then(gameUi.onGetGameSuccess).catch(gameUi.onGetGameFailure);
};
...
var onGetFinishedGames = function onGetFinishedGames(event) {
  event.preventDefault();
  gameApi.getFinishedGames().then(gameUi.onGetFinishedGamesSuccess).catch(gameUi.onGetFinishedGamesFailure);
};
module.exports = {
  click: click, // export to index.js
  onNewGame: onNewGame,
  onGetGame: onGetGame,
...
var checkWinner = function checkWinner(gameArray) {
...
var getFormFields = __webpack_require__(7);
var userApi = __webpack_require__(16);
var userUi = __webpack_require__(17);
...
var onSignUp = function onSignUp(event) {
  ...
  if (user.email.length !== 0) {
    if (user.password === user.password_confirmation) {
      userApi.signUp(data).then(userUi.onSignUpSuccess).catch(userUi.onSignUpFailure);
      // userApi.signIn(data)
